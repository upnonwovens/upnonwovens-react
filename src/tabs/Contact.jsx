...
const Contact = () => {
  const managementTeam = [
    { name: "MRS. PRIYA ARYA", role: "EXECUTIVE DIRECTOR", bio: "Priya Arya serves as Executive Director, steering structural corporate planning and strategic business expansion frameworks.", image: "/priya.jpg" },
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '40px auto', padding: '0 20px', boxSizing: 'border-box' }}>
      ...
      {/* Leadership Section */}
      <section style={{ marginBottom: '60px', paddingBottom: '40px' }}>
        <h2 style={{ fontSize: '26px', fontWeight: '800', color: '#0f172a', marginBottom: '40px', textAlign: 'center', letterSpacing: '0.5px' }}>OUR MANAGEMENT TEAM</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '30px', justifyContent: 'center' }}>
          {managementTeam.map((member, index) => (
            <div key={index} style={{ backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: '16px', border: '1px solid rgba(226, 232, 240, 0.8)', overflow: 'hidden', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.05)', textAlign: 'center', padding: '30px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', backdropFilter: 'blur(10px)' }}>
              <div style={{ width: '140px', height: '140px', borderRadius: '50%', overflow: 'hidden', marginBottom: '20px', backgroundColor: '#e2e8f0', border: '2px solid #cbd5e1', position: 'relative' }}>
                <img src={member.image} alt={member.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.target.style.display = 'none'; }} />
              </div>
              <h3 style={{ fontSize: '17px', fontWeight: '800', color: '#0f172a', margin: '0 0 4px 0', letterSpacing: '0.5px' }}>{member.name}</h3>
              <p style={{ color: '#2563eb', fontSize: '13px', fontWeight: '700', margin: '0 0 15px 0' }}>{member.role}</p>
              <p style={{ color: '#475569', fontSize: '14px', lineHeight: '1.6', margin: 0, maxWidth: '280px' }}>{member.bio}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* SINGLE UNIFIED MASTER CARD: Housing Facility Info & Live GPS Map */}
      <section style={{ marginBottom: '60px' }}>
        <div style={{ 
          ...
          {/* Left Column: Coordinates & Communications */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '30px', justifyContent: 'center' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                ...
                <h3 style={{ fontSize: '20px', fontWeight: '800', margin: 0, color: '#0f172a' }}>Facility Coordinates</h3>
              </div>
              <p style={{ color: '#475569', fontSize: '15px', lineHeight: '1.7', margin: 0, paddingLeft: '34px' }}>
            ...
            <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '25px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', color: '#475569', fontSize: '15px', paddingLeft: '34px' }}>
                <div>
                  <span style={{ display: 'block', fontWeight: '700', color: '#334155', marginBottom: '2px' }}>Email Enquiries:</span>
                  <span style={{ display: 'block' }}>upnonwovens@gmail.com</span>
                  <span style={{ display: 'block' }}>sale.ksfnonwoven@gmail.com</span>
                </div>
                
                <div>
                  <span style={{ display: 'block', fontWeight: '700', color: '#334155', marginBottom: '2px' }}>Direct Contacts:</span>
          ...
          {/* Right Column: Live Interactive GPS Map Box */}
          <div style={{ 
            borderRadius: '16px', 
            overflow: 'hidden', 
